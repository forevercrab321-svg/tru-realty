// GENERATED by gateway/build-gateway.mjs — edit gateway/src/index.ts instead.
...
var kimiBase = (env) => (env.KIMI_BASE_URL ?? DEFAULT_KIMI_BASE).replace(/\/+$/, "");
var DEMO_ACCOUNTS = {
...
async function underLimit(env, key, ceiling) {
...
async function handle(req, env) {
  if (new URL(req.url).pathname === "/health") {
    ...
    if (env.KIMI_API_KEY) {
      ...
      if (idx >= 0) {
        ...
        report.verdict = wu === base && wm === (env.KIMI_MODEL || "k3") ? `Working. KIMI_BASE_URL and KIMI_MODEL are correct.` : `Key works on ${wu} with model "${wm}". Set KIMI_BASE_URL="${wu}" and KIMI_MODEL="${wm}" and redeploy.`;
      } else if (anyHtml) {
  ...
  if (url.pathname === "/session") {
    ...
    const cookie = await signSession(env, { ...account, exp: Math.floor(Date.now() / 1e3) + 12 * 3600 });
...
var src_default = { fetch: handle };
async function flush(env, entries) {
