            ...
            <div className="flex items-center justify-between">
              ...
              <span
                className={cn(
                  ...
                  d.tone === "surface" ? "text-ink-4" : "text-white/45"
                )}
              >
                {d.eyebrow}
              </span>
    ...
    <section className="border-y border-line bg-ink">
      <div className="mx-auto max-w-[1280px] px-5 py-16 sm:px-8 lg:py-24">
        ...
        <div className="mt-12 grid gap-x-8 gap-y-9 sm:grid-cols-2 lg:grid-cols-3">
          {modules.map((m) => (
            <div key={m.name}>
              <span className="mb-3.5 flex size-9 items-center justify-center rounded-[10px] bg-white/[0.07] text-brand-300 [&_svg]:size-[18px]">
                {m.icon}
              </span>
              <p className="text-[15px] font-semibold tracking-[-0.015em] text-white">{m.name}</p>
              <p className="mt-1.5 text-[13.5px] leading-relaxed text-white/55">{m.body}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-12 flex flex-wrap items-center gap-x-8 gap-y-3 border-t border-white/10 pt-7">
          ...
          {[
            ...
            <span key={email} className="flex items-center gap-2 text-[12.5px]">
              <code className="rounded-[5px] bg-white/[0.07] px-1.5 py-0.5 font-mono text-white/80">
                {email}
              </code>
